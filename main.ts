...
function getPoolContract(provider: Provider): Contract {
	...
	return new ethers.Contract(currentPoolAddress, IUniswapV3PoolABI.abi, provider);
}
...
async function getConfiguredPool(poolContract: Contract): Promise<Pool> {
	...
	return new Pool(tokenA, tokenB, poolFee, slot0.sqrtPriceX96.toString(), liquidity.toString(), Number(slot0.tick));
}
...
async function supplyAndApproveTokens(
	...
	// mint and transfer
	const [mintAtx, mintBtx] = await Promise.all([erc20Acontract.mint(amountA), erc20Bcontract.mint(amountB)]);
...
function buildMintTransaction(from: string, position: Position): TransactionRequest {
	const { calldata, value } = NonfungiblePositionManager.addCallParameters(position, getMintOptions(from));
